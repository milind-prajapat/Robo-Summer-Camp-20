...
def Collect_Observation(Id):
    (x, y, z), Orn = p.getBasePositionAndOrientation(Id)
    roll, pitch, yaw = p.getEulerFromQuaternion(Orn)
    vel, _ = p.getBaseVelocity(Id)
    
    walk_target_theta = np.arctan2(0 - y, 1000 - x)
    angle_to_target = walk_target_theta - yaw
    rot_speed = np.array([[np.cos(-yaw), -np.sin(-yaw), 0],[np.sin(-yaw), np.cos(-yaw), 0], [0, 0, 1]], dtype = np.float32)
    
    vx, vy, vz = np.dot(rot_speed, vel)
    data_set_1 = np.array([z - 0.8, np.sin(angle_to_target), np.cos(angle_to_target), 0.3 * vx, 0.3 * vy, 0.3 * vz, roll, pitch], dtype = np.float32)

    joint_states = p.getJointStates(Id, joint_indices)
    data_set_2 = np.array([Current_Relative_Position(Id, joint_states, *i) for i in joints], dtype = np.float32).flatten()
    
    right_foot_contact = p.getContactPoints(Id, -1, right_foot, -1)
    left_foot_contact = p.getContactPoints(Id, -1, left_foot, -1)
    data_set_3 = np.array([bool(len(right_foot_contact) > 0),bool(len(left_foot_contact) > 0)], dtype = np.float32)
...
def Simulation():
    ...
    while 1:
        ...
        output_data = np.clip(nn.Output(data), -1, +1)
        forces = [0.]*len(motors)
        for i in range(len(motors)): forces[i] = motor_power[i] * output_data[i] * 0.41
