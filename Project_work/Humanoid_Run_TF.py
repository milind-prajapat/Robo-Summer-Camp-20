...
c_id = p.connect(p.SHARED_MEMORY)
if(c_id < 0):
    ''' If fails to connect with shared memory, then connects with gui '''
    c_id = p.connect(p.GUI)

p.setAdditionalSearchPath(pybullet_data.getDataPath())
p.setGravity(0, 0, -9.8)
p.setPhysicsEngineParameter(fixedTimeStep = 1.0/60.0, numSolverIterations = 50, numSubSteps = 4)

p.loadURDF("plane.urdf")
Human = p.loadMJCF("mjcf/humanoid_symmetric_no_ground.xml", flags = p.URDF_USE_SELF_COLLISION_EXCLUDE_ALL_PARENTS)

Id = Human[0]
(_, _, z_initial), _ = p.getBasePositionAndOrientation(Id)
joints = []
joint_indices = []
joint_dict = {}
...
for i in range(p.getNumJoints(Id)):
    ...
    if link_name == "left_foot": left_foot = i
    if link_name == "right_foot": right_foot = i
    ...
    lower = info[8]
    upper = info[9]
    joints.append((i, lower, upper))

    p.setJointMotorControl2(Id, i, controlMode = p.VELOCITY_CONTROL, force = 0)
...
motors = [joint_dict[i] for i in motor_names]

def Current_Relative_Position(Id, joint_states, joint_Id, lower, upper):
    joint = joint_states[joint_Id]
    pos = joint[0]
    vel = joint[1]
    mid_pos = (lower + upper) * 0.5
    
    return (2 * (pos - mid_pos) / (upper - lower), 0.1 * vel)

def Collect_Observation(Id):
    global z_initial
    (_, _, z), Orn = p.getBasePositionAndOrientation(Id)
    (vx, vy, vz), _ = p.getBaseVelocity(Id)    
    data_set_1 = np.array([z - z_initial, 0.1 * vx, 0.1 * vy, 0.1 * vz, *Orn])

    joint_states = p.getJointStates(Id, joint_indices)
    data_set_2 = np.array([Current_Relative_Position(Id, joint_states, *i) for i in joints]).flatten()

    right_foot_contact = p.getContactPoints(Id, -1, right_foot, -1)
    left_foot_contact = p.getContactPoints(Id, -1, left_foot, -1)
    data_set_3 = np.array([len(right_foot_contact) > 0,len(left_foot_contact) > 0])
    
    return np.clip(np.concatenate([data_set_1]+[data_set_2]+[data_set_3]), -5, +5)

class Neural_Network():
    def __init__(self, name):
        with tf.variable_scope(name):
            ''' This is to reuse variables once created '''
            self.Input = tf.placeholder(dtype = tf.float32, shape = [None, 44])
            ''' This is to access the variable self.input from anywhere '''
            x = self.Input

            self.dense1_w = tf.get_variable("dense1_w", [x.get_shape()[1], 256])
            self.dense1_b = tf.get_variable("dense1_b", [256])
            x = tf.nn.relu(tf.matmul(x, self.dense1_w) + self.dense1_b)

            self.dense2_w = tf.get_variable("dense2_w", [x.get_shape()[1], 128])
            self.dense2_b = tf.get_variable("dense2_b", [128])
            x = tf.nn.relu(tf.matmul(x, self.dense2_w) + self.dense2_b)

            self.final_w = tf.get_variable("final_w", [x.get_shape()[1], 17])
            self.final_b = tf.get_variable("final_b", [17])
            x = tf.matmul(x, self.final_w) + self.final_b

            self.output = x

    def Output(self, Input):
        out = tf.get_default_session().run(self.output, feed_dict = {self.Input: Input[None]})
        ''' This is to feed variable self.input and to get value of self.output '''
        return out[0]

    def Load_Weights(self):   
        assigns = [(self.dense1_w, dense1_weights), (self.dense1_b, dense1_biases),
                   (self.dense2_w, dense2_weights), (self.dense2_b, dense2_biases),
                   (self.final_w, final_weights), (self.final_b, final_biases)]
        to_run = []
        feed_dict = {}
        for i,j in assigns:
            temp = tf.placeholder(tf.float32, j.shape)
            to_run.append(tf.assign(i,temp))
            ''' This will make i accessible from anywhere '''
            feed_dict[temp] = j
        tf.get_default_session().run(to_run, feed_dict = feed_dict)
        ''' Here feed_dict will feed i with j '''
...
def Simulation():
    ...
    frame = 0
    while frame!=1000:
        data = Collect_Observation(Id)
        output_data = np.clip(nn.Output(data), -15, 15)
        forces = [0.]*len(motors)
        for i in range(len(motors)): forces[i] = motor_power[i] * output_data[i] * 0.082
